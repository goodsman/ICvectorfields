...
The functionality of *ICvectorfields* is demonstrated using one simulated dataset and one empirical dataset both of which are embedded in the package.
...
# Note that terra does not recognize the result as 
# SpatRaster type so the dim() function does not work.
dim(SimStack)

# ensuring that terra recognises output as rast
SimStack <- terra::rast(SimStack)
dim(SimStack)
...
To decide which ICvectorfields function to use, the user can consult the table in the overview section of this vignette. We will first use the *DispField* funtion look displacement based on a pair of images using a grid to
...
The movement speed is estimated as 0.196 in each of the quadrats and the directions are consistent with simulated advection directions. Note that in the function above, restricted is set to TRUE, whereas the default is FALSE. The restricted argument restricts the search for cross-covariance to areas within each of the grids that are designated using the factv1, and facth1 arguments to the function when set to TRUE. When restricted is set to false the algorithm searches the entire spatial domain to look for maximum cross-covariance. 
...
Plotting vector fields capitlizes on ggplot extensions in the metR package:
...
# convert to raster stack
LBMStack <- ICvectorfields::RastStackData(LBMfromncfpkg)
...
LBMVF = ggplot() +
  geom_raster(data = LBMfromncfpkg,
              aes(x = x, y = y, fill = X1963)) +
...
Jhonson et al noticed that at smaller spatial scales there appeared to be two outbreak epicentres and that outbreaks spread outwards from each of epicentre. Multiple epicentres become evident when analysis is restricted to a maximum two year time lag.

```{r, fig.width = 7, fig.height = 5}
# VFdf4 <- DispFieldSTall(LBMStack[[1:23]], lagmax = 2, factv1 = 3, facth1 = 3, restricted = FALSE)
VFdf4 <- DispFieldST(LBMStack[[1:23]], lag1 = 1, factv1 = 3, facth1 = 3, restricted = FALSE)

LBMVF2 = ggplot() +
  geom_raster(data = LBMfromncfpkg,
              aes(x = x, y = y, fill = X1963)) +
  geom_vector(data = VFdf4, 
              aes(x = centx, y = centy, 
                  mag = Mag(dispx, dispy), 
                  angle = Angle(dispx, dispy)),
              size = 1,
